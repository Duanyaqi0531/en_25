...
void Class_DJI_Motor_GM6020::Data_Process()
{
    ...
    //记录上电初始位置的编码值
    static int16_t tmp_start_encoder = 0; 
    if(Start_Falg==0)
    {
        tmp_start_encoder = tmp_encoder;
    }
    //计算圈数与总编码器值
    if(Start_Falg==1)
    {
    ...
    //计算电机本身信息
     Data.Now_Angle = (float)(Data.Total_Encoder - tmp_start_encoder)/ (float)Encoder_Num_Per_Round * 360.0f;
     Data.Now_Radian = (float)(Data.Total_Encoder - tmp_start_encoder)/ (float)Encoder_Num_Per_Round * 2.0f * PI;
...
void Class_DJI_Motor_C610::Data_Process()
{
    ...
    //计算电机本身信息
    //注意：此处计算的是：零位的角度
    Data.Now_Angle = (float)(Data.Total_Encoder - tmp_start_encoder) / (float)Encoder_Num_Per_Round *360.f / Gearbox_Rate;
    Data.Now_Radian = (float)(Data.Total_Encoder - tmp_start_encoder) / (float)Encoder_Num_Per_Round * 2.0f * PI  / Gearbox_Rate;
    Data.Now_Omega_Radian = (float)tmp_omega * RPM_TO_RADPS / Gearbox_Rate;
    Data.Now_Omega_Angle = (float)tmp_omega * RPM_TO_DEG / Gearbox_Rate;
    Data.Now_Torque = tmp_torque;
    Data.Now_Temperature = tmp_temperature + CELSIUS_TO_KELVIN;
